...
# -----------------------------
# CSV Loader (FIXED: Returns tuple of (rows, fieldnames))
# -----------------------------
...
# -----------------------------
# Parse KML and extract routes (Unchanged)
# -----------------------------
def extract_routes_from_kml(kml_path: str) -> List[Tuple[str, RouteCoords]]:
    ...
    def scan_node(node, current_path=""):
        ...
        if tag_name in ("folder", "document"):
            ...
            if all_coords:
                # print(f"  ➤ Found Route: {full_name} ({len(all_coords)} points)")
...
# -----------------------------
# Compute nearest point on route (Unchanged)
# -----------------------------
def compute_nearest_point(lat: float, lon: float, coords: RouteCoords) -> Tuple[float, Tuple[float, float]]:
    """Tính khoảng cách gần nhất từ điểm (lat, lon) đến tuyến đường (coords)."""
    # coords là list [(lon, lat), ...]
    if len(coords) < 2:
         # Tránh lỗi LineString khi chỉ có 0 hoặc 1 điểm
        return float('inf'), (lat, lon) 
        
    line = LineString(coords) 
    p = Point(lon, lat) # Point nhận (lon, lat)
...
# -----------------------------
# Helper: Find the single nearest route for one point (Unchanged)
# -----------------------------
...
# -----------------------------
# Main Process (UPDATED: Separate P1/P2 results and add short name)
# -----------------------------
def process_kml(kml_path: str, csv_path: str, output_excel: str):
    ...
    print("\n🔍 Bắt đầu tính toán khoảng cách cho từng cặp điểm...")
    ...
    for i, row_data in enumerate(point_rows):
        ...
        dist1 = f"{result1['distance']:.2f}"
        lat1_n = f"{result1['nearest_lat']:.6f}"
        lon1_n = f"{result1['nearest_lon']:.6f}"
        
        dist2 = f"{result2['distance']:.2f}"
        lat2_n = f"{result2['nearest_lat']:.6f}"
        lon2_n = f"{result2['nearest_lon']:.6f}"
